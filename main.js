const form = document.getElementById('quiz');                                                             
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const scores = questions.map(q => {
      ...
      return checked?.value === 'other' ? 2 : parseInt(checked?.value || 0);
    });
    ...
    questions.forEach(q => {
      ...
      if (checked?.value === 'other') {
        answers[q] = form.querySelector(`textarea[name="${q}_text"]`)?.value || 'その他';
      } else {
        answers[q] = checked?.parentElement.textContent.trim();
      }
    });
    ...
    try {
      ...
      const tweetText = encodeURIComponent(
        `私の職場の地獄度は
  ${hellPercent}%「${getHellName(hellPercent)}」でした🔥\n\n#職場地獄度診断\n${APP_URL}`
      );
